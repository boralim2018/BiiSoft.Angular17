...
export class RAMComponent extends Mixin(PrimeNgListComponentBase<RAMListDto>, ExcelFileComponentBase, NavBarComponentBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('ramTable') table: Table;
    canCreate: boolean = this.isGranted(AppPermissions.pages.setup.items.rams.create);
    ...
    canView: boolean = this.isGranted(AppPermissions.pages.setup.items.rams.view);
    ...
    canImportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.rams.importExcel);
    canExportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.rams.exportExcel);
    ...
    ngOnInit() {
        this.tableCacheKey = "RAMTableCacheKey";
        super.ngOnInit();
        this.initNavBar();
    ...
    initNavBar() {
        this.title = this.l("RAMs");
        this.setTitle();
    }

    private initActionMenuItems() {
        this.actionMenuItems = [];
        if (this.canCreate) this.actionMenuItems.push({ label: this.l('Create'), icon: 'pi pi-plus-circle', command: () => { this.createRAM(); } });
    ...
    delete(ram: RAMListDto): void {
        ...
        .onClose.subscribe(result => {
            if (result) {
                ...
                this._ramService.delete(ram.id)
    ...
    importExcel() {
        ...
        instance.upload.subscribe(result => {
            if (result) {
                ...
                this._ramService.importExcel(fileInput)
        ...
        instance.download.subscribe(result => {
            instance.loading = true;
            this._ramService.exportExcelTemplate()
                .pipe(finalize(() => instance.loading = false))
                .subscribe(result => {
    ...
    exportExcel() {
        ...
        let input = new ExportExcelRAMInputDto();
        ...
        this._ramService
            ...
            .subscribe((result: ExportFileOutput) => {
                this.downloadExcel(AppConsts.remoteServiceBaseUrl + result.fileUrl, `RAM_${moment().format("YYYY-MM-DD-HH-mm-ss")}.xlsx`);
            });
    }

    editRAM(ram: RAMListDto): void {
        this.showCreateOrEditUserDialog(ram.id);
    }

    showCreateOrEditUserDialog(id?: string): void {
        let createOrEditUserDialog: DynamicDialogRef;
        if (!id) {
            createOrEditUserDialog = this._dialogService.open(CreateRAMComponent, {
                data: {},
                header: this.l('Create') + ' ' + this.l('RAM'),
                styleClass: this.responsiveDialogClass
            });
        } else {
            createOrEditUserDialog = this._dialogService.open(EditRAMComponent, {
                data: { id: id },
    ...
    enable(ram: RAMListDto) {
        this.message.confirm(
            this.l('EnableWarningMessage', ram.name), this.l('Enable'), (result) => {
                if (result) {
    ...
    disable(ram: RAMListDto) {
        this.message.confirm(
            this.l('DisableWarningMessage', ram.name), this.l('Disable'), (result) => {
                if (result) {
    ...
    setAsDefault(ram: RAMListDto) {
        this.message.confirm(
            this.l('DefaultWarningMessage', ram.name), this.l('SetAsDefault'), (result) => {
                if (result) {
    ...
    showInlineActions(ram: RAMListDto, event: Event) {
        if (!this.inlineActionMenu) return;

        this.inlineActionMenu.model = [];
        if (this.canView) this.inlineActionMenu.model.push({ label: this.l('View'), icon: 'pi pi-fw pi-eye', command: () => { this.viewDetail(ram); } });
        if (this.canEdit) this.inlineActionMenu.model.push({ label: this.l('Edit'), icon: 'pi pi-fw pi-pencil', command: () => { this.editRAM(ram); } });
        if (this.canDelete) this.inlineActionMenu.model.push({ label: this.l('Delete'), icon: 'pi pi-trash', command: () => { this.delete(ram); } });
