...
import {
    ItemSizeServiceProxy,
    ItemSizeListDto,
    ExportExcelItemSizeInputDto,
    ColumnOutput,
    ExportFileOutput,
    FileTokenInput,
...
import { CreateItemSizeComponent } from './create-item-size/create-item-size.component';
...
export class ItemSizeComponent extends Mixin(PrimeNgListComponentBase<ItemSizeListDto>, ExcelFileComponentBase, NavBarComponentBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('itemSizeTable') table: Table;
    canCreate: boolean = this.isGranted(AppPermissions.pages.setup.items.sizes.create);
    ...
    canView: boolean = this.isGranted(AppPermissions.pages.setup.items.sizes.view);
    ...
    canImportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.sizes.importExcel);
    canExportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.sizes.exportExcel);
    ...
    ngOnInit() {
        this.tableCacheKey = "ItemSizeTableCacheKey";
        super.ngOnInit();
        this.initNavBar();
    ...
    initNavBar() {
        this.title = this.l("ItemSizes");
        this.setTitle();
    }

    private initActionMenuItems() {
        this.actionMenuItems = [];
        if (this.canCreate) this.actionMenuItems.push({ label: this.l('Create'), icon: 'pi pi-plus-circle', command: () => { this.createItemSize(); } });
    ...
    delete(itemSize: ItemSizeListDto): void {

        this._dialogService.open(ConfirmDeleteComponent, {
            data: {
                deleteObj: itemSize.name,
        ...
        .onClose.subscribe(result => {
            if (result) {
                ...
                this._itemSizeService.delete(itemSize.id)
    ...
    importExcel() {
        ...
        instance.upload.subscribe(result => {
            if (result) {
                ...
                this._itemSizeService.importExcel(fileInput)
        ...
        instance.download.subscribe(result => {
            instance.loading = true;
            this._itemSizeService.exportExcelTemplate()
                .pipe(finalize(() => instance.loading = false))
                .subscribe(result => {
    ...
    exportExcel() {
        ...
        let input = new ExportExcelItemSizeInputDto();
        ...
        this._itemSizeService
            ...
            .subscribe((result: ExportFileOutput) => {
                this.downloadExcel(AppConsts.remoteServiceBaseUrl + result.fileUrl, `ItemSize_${moment().format("YYYY-MM-DD-HH-mm-ss")}.xlsx`);
            });
    }

    editItemSize(itemSize: ItemSizeListDto): void {
        this.showCreateOrEditUserDialog(itemSize.id);
    }

    showCreateOrEditUserDialog(id?: string): void {
        let createOrEditUserDialog: DynamicDialogRef;
        if (!id) {
            createOrEditUserDialog = this._dialogService.open(CreateItemSizeComponent, {
                data: {},
                header: this.l('Create') + ' ' + this.l('ItemSize'),
                styleClass: this.responsiveDialogClass
            });
        } else {
            createOrEditUserDialog = this._dialogService.open(EditItemSizeComponent, {
                data: { id: id },
    ...
    enable(itemSize: ItemSizeListDto) {
        this.message.confirm(
            this.l('EnableWarningMessage', itemSize.name), this.l('Enable'), (result) => {
                if (result) {
    ...
    disable(itemSize: ItemSizeListDto) {
        this.message.confirm(
            this.l('DisableWarningMessage', itemSize.name), this.l('Disable'), (result) => {
                if (result) {
    ...
    setAsDefault(itemSize: ItemSizeListDto) {
        this.message.confirm(
            this.l('DefaultWarningMessage', itemSize.name), this.l('SetAsDefault'), (result) => {
                if (result) {
    ...
    showInlineActions(itemSize: ItemSizeListDto, event: Event) {
        if (!this.inlineActionMenu) return;

        this.inlineActionMenu.model = [];
        if (this.canView) this.inlineActionMenu.model.push({ label: this.l('View'), icon: 'pi pi-fw pi-eye', command: () => { this.viewDetail(itemSize); } });
        if (this.canEdit) this.inlineActionMenu.model.push({ label: this.l('Edit'), icon: 'pi pi-fw pi-pencil', command: () => { this.editItemSize(itemSize); } });
        if (this.canDelete) this.inlineActionMenu.model.push({ label: this.l('Delete'), icon: 'pi pi-trash', command: () => { this.delete(itemSize); } });
