...
export class ScreenComponent extends Mixin(PrimeNgListComponentBase<ScreenListDto>, ExcelFileComponentBase, NavBarComponentBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('screenTable') table: Table;
    canCreate: boolean = this.isGranted(AppPermissions.pages.setup.items.screens.create);
    ...
    canView: boolean = this.isGranted(AppPermissions.pages.setup.items.screens.view);
    ...
    canImportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.screens.importExcel);
    canExportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.screens.exportExcel);
    ...
    ngOnInit() {
        this.tableCacheKey = "ScreenTableCacheKey";
        super.ngOnInit();
        this.initNavBar();
    ...
    initNavBar() {
        this.title = this.l("Screens");
        this.setTitle();
    }

    private initActionMenuItems() {
        this.actionMenuItems = [];
        if (this.canCreate) this.actionMenuItems.push({ label: this.l('Create'), icon: 'pi pi-plus-circle', command: () => { this.createScreen(); } });
    ...
    delete(screen: ScreenListDto): void {

        this._dialogService.open(ConfirmDeleteComponent, {
            data: {
                deleteObj: screen.name,
        ...
        .onClose.subscribe(result => {
            if (result) {
                ...
                this._screenService.delete(screen.id)
    ...
    importExcel() {
        ...
        instance.upload.subscribe(result => {
            if (result) {
                ...
                this._screenService.importExcel(fileInput)
        ...
        instance.download.subscribe(result => {
            instance.loading = true;
            this._screenService.exportExcelTemplate()
                .pipe(finalize(() => instance.loading = false))
                .subscribe(result => {
    ...
    exportExcel() {
        ...
        let input = new ExportExcelScreenInputDto();
        ...
        this._screenService
            ...
            .subscribe((result: ExportFileOutput) => {
                this.downloadExcel(AppConsts.remoteServiceBaseUrl + result.fileUrl, `Screen_${moment().format("YYYY-MM-DD-HH-mm-ss")}.xlsx`);
            });
    }

    editScreen(screen: ScreenListDto): void {
        this.showCreateOrEditUserDialog(screen.id);
    }

    showCreateOrEditUserDialog(id?: string): void {
        let createOrEditUserDialog: DynamicDialogRef;
        if (!id) {
            createOrEditUserDialog = this._dialogService.open(CreateScreenComponent, {
                data: {},
                header: this.l('Create') + ' ' + this.l('Screen'),
                styleClass: this.responsiveDialogClass
            });
        } else {
            createOrEditUserDialog = this._dialogService.open(EditScreenComponent, {
                data: { id: id },
    ...
    enable(screen: ScreenListDto) {
        this.message.confirm(
            this.l('EnableWarningMessage', screen.name), this.l('Enable'), (result) => {
                if (result) {
    ...
    disable(screen: ScreenListDto) {
        this.message.confirm(
            this.l('DisableWarningMessage', screen.name), this.l('Disable'), (result) => {
                if (result) {
    ...
    setAsDefault(screen: ScreenListDto) {
        this.message.confirm(
            this.l('DefaultWarningMessage', screen.name), this.l('SetAsDefault'), (result) => {
                if (result) {
    ...
    showInlineActions(screen: ScreenListDto, event: Event) {
        if (!this.inlineActionMenu) return;

        this.inlineActionMenu.model = [];
        if (this.canView) this.inlineActionMenu.model.push({ label: this.l('View'), icon: 'pi pi-fw pi-eye', command: () => { this.viewDetail(screen); } });
        if (this.canEdit) this.inlineActionMenu.model.push({ label: this.l('Edit'), icon: 'pi pi-fw pi-pencil', command: () => { this.editScreen(screen); } });
        if (this.canDelete) this.inlineActionMenu.model.push({ label: this.l('Delete'), icon: 'pi pi-trash', command: () => { this.delete(screen); } });
