...
import { CreateFieldAComponent } from './create-field-a/create-field-a.component';
...
@Component({
    selector: 'app-field-a',
    templateUrl: './field-a.component.html',
...
export class FieldAComponent extends Mixin(PrimeNgListComponentBase<FieldAListDto>, ExcelFileComponentBase, NavBarComponentBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('fieldATable') table: Table;
    canCreate: boolean = this.isGranted(AppPermissions.pages.setup.items.fieldAs.create);
    ...
    canView: boolean = this.isGranted(AppPermissions.pages.setup.items.fieldAs.view);
    ...
    canImportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.fieldAs.importExcel);
    canExportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.fieldAs.exportExcel);
    ...
    ngOnInit() {
        this.tableCacheKey = "FieldATableCacheKey";
        super.ngOnInit();
        this.initNavBar();
    ...
    initNavBar() {
        this.title = this.l("FieldAs");
        this.setTitle();
    }

    private initActionMenuItems() {
        this.actionMenuItems = [];
        if (this.canCreate) this.actionMenuItems.push({ label: this.l('Create'), icon: 'pi pi-plus-circle', command: () => { this.createFieldA(); } });
    ...
    protected getList(input: any, callBack: Function) {
        ...
        this._fieldAService
            .getList(input.isActive, input.creatorFilter.exclude, input.creatorFilter.ids, input.modifierFilter.exclude, input.modifierFilter.ids, input.keyword, input.sortField, input.sortMode, input.usePagination, input.skipCount, input.maxResultCount)
    ...
    delete(fieldA: FieldAListDto): void {

        this._dialogService.open(ConfirmDeleteComponent, {
            data: {
                deleteObj: fieldA.name,
        ...
        .onClose.subscribe(result => {
            if (result) {
                ...
                this._fieldAService.delete(fieldA.id)
    ...
    importExcel() {
        ...
        instance.upload.subscribe(result => {
            if (result) {
                ...
                this._fieldAService.importExcel(fileInput)
        ...
        instance.download.subscribe(result => {
            instance.loading = true;
            this._fieldAService.exportExcelTemplate()
                .pipe(finalize(() => instance.loading = false))
                .subscribe(result => {
    ...
    exportExcel() {
        ...
        let input = new ExportExcelFieldAInputDto();
        ...
        this._fieldAService
            ...
            .subscribe((result: ExportFileOutput) => {
                this.downloadExcel(AppConsts.remoteServiceBaseUrl + result.fileUrl, `FieldA_${moment().format("YYYY-MM-DD-HH-mm-ss")}.xlsx`);
            });
    }

    editFieldA(fieldA: FieldAListDto): void {
        this.showCreateOrEditUserDialog(fieldA.id);
    }

    showCreateOrEditUserDialog(id?: string): void {
        let createOrEditUserDialog: DynamicDialogRef;
        if (!id) {
            createOrEditUserDialog = this._dialogService.open(CreateFieldAComponent, {
                data: {},
                header: this.l('Create') + ' ' + this.l('FieldA'),
                styleClass: this.responsiveDialogClass
            });
        } else {
            createOrEditUserDialog = this._dialogService.open(EditFieldAComponent, {
                data: { id: id },
    ...
    enable(fieldA: FieldAListDto) {
        this.message.confirm(
            this.l('EnableWarningMessage', fieldA.name), this.l('Enable'), (result) => {
                if (result) {
    ...
    disable(fieldA: FieldAListDto) {
        this.message.confirm(
            this.l('DisableWarningMessage', fieldA.name), this.l('Disable'), (result) => {
                if (result) {
    ...
    setAsDefault(fieldA: FieldAListDto) {
        this.message.confirm(
            this.l('DefaultWarningMessage', fieldA.name), this.l('SetAsDefault'), (result) => {
                if (result) {
    ...
    unsetAsDefault(fieldA: FieldAListDto) {
        this.message.confirm(
            this.l('DefaultWarningMessage', fieldA.name), this.l('UnsetAsDefault'), (result) => {
                if (result) {
    ...
    viewDetail(fieldA: FieldAListDto) {
        this._router.navigate(['/app/main/field-a/view-detail', fieldA.id]);
    }

    showInlineActions(fieldA: FieldAListDto, event: Event) {
        if (!this.inlineActionMenu) return;

        this.inlineActionMenu.model = [];
        if (this.canView) this.inlineActionMenu.model.push({ label: this.l('View'), icon: 'pi pi-fw pi-eye', command: () => { this.viewDetail(fieldA); } });
        if (this.canEdit) this.inlineActionMenu.model.push({ label: this.l('Edit'), icon: 'pi pi-fw pi-pencil', command: () => { this.editFieldA(fieldA); } });
        if (this.canDelete) this.inlineActionMenu.model.push({ label: this.l('Delete'), icon: 'pi pi-trash', command: () => { this.delete(fieldA); } });
        if (this.canEnable && !fieldA.isActive) this.inlineActionMenu.model.push({ label: this.l('Enable'), icon: 'pi pi-check', command: () => { this.enable(fieldA); } });
        if (this.canDisable && fieldA.isActive) this.inlineActionMenu.model.push({ label: this.l('Disable'), icon: 'pi pi-ban', command: () => { this.disable(fieldA); } });
        if (this.canSetAsDefault && !fieldA.isDefault) this.inlineActionMenu.model.push({ label: this.l('SetAsDefault'), icon: 'fa-solid fa-check-double', command: () => { this.setAsDefault(fieldA); } });
        if (this.canSetAsDefault && fieldA.isDefault) this.inlineActionMenu.model.push({ label: this.l('UnsetAsDefault'), icon: 'fa-solid fa-check-double', command: () => { this.unsetAsDefault(fieldA); } });
