...
import {
    ColorPatternServiceProxy,
    ColorPatternListDto,
    ExportExcelColorPatternInputDto,
    ColumnOutput,
    ExportFileOutput,
    FileTokenInput,
...
import { CreateColorPatternComponent } from './create-color-pattern/create-color-pattern.component';
...
@Component({
    selector: 'app-color-pattern',
    templateUrl: './color-pattern.component.html',
...
export class ColorPatternComponent extends Mixin(PrimeNgListComponentBase<ColorPatternListDto>, ExcelFileComponentBase, NavBarComponentBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('colorPatternTable') table: Table;
    canCreate: boolean = this.isGranted(AppPermissions.pages.setup.items.colorPatterns.create);
    ...
    canView: boolean = this.isGranted(AppPermissions.pages.setup.items.colorPatterns.view);
    ...
    canImportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.colorPatterns.importExcel);
    canExportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.colorPatterns.exportExcel);
    ...
    ngOnInit() {
        this.tableCacheKey = "ColorPatternTableCacheKey";
        super.ngOnInit();
        this.initNavBar();
    ...
    initNavBar() {
        this.title = this.l("ColorPatterns");
        this.setTitle();
    }

    private initActionMenuItems() {
        this.actionMenuItems = [];
        if (this.canCreate) this.actionMenuItems.push({ label: this.l('Create'), icon: 'pi pi-plus-circle', command: () => { this.createColorPattern(); } });
    ...
    delete(colorPattern: ColorPatternListDto): void {

        this._dialogService.open(ConfirmDeleteComponent, {
            data: {
                deleteObj: colorPattern.name,
        ...
        .onClose.subscribe(result => {
            if (result) {
                ...
                this._colorPatternService.delete(colorPattern.id)
    ...
    importExcel() {
        ...
        instance.upload.subscribe(result => {
            if (result) {
                ...
                this._colorPatternService.importExcel(fileInput)
        ...
        instance.download.subscribe(result => {
            instance.loading = true;
            this._colorPatternService.exportExcelTemplate()
                .pipe(finalize(() => instance.loading = false))
                .subscribe(result => {
    ...
    exportExcel() {
        ...
        let input = new ExportExcelColorPatternInputDto();
        ...
        this._colorPatternService
            ...
            .subscribe((result: ExportFileOutput) => {
                this.downloadExcel(AppConsts.remoteServiceBaseUrl + result.fileUrl, `ColorPattern_${moment().format("YYYY-MM-DD-HH-mm-ss")}.xlsx`);
            });
    }

    editColorPattern(colorPattern: ColorPatternListDto): void {
        this.showCreateOrEditUserDialog(colorPattern.id);
    }

    showCreateOrEditUserDialog(id?: string): void {
        let createOrEditUserDialog: DynamicDialogRef;
        if (!id) {
            createOrEditUserDialog = this._dialogService.open(CreateColorPatternComponent, {
                data: {},
                header: this.l('Create') + ' ' + this.l('ColorPattern'),
                styleClass: this.responsiveDialogClass
            });
        } else {
            createOrEditUserDialog = this._dialogService.open(EditColorPatternComponent, {
                data: { id: id },
    ...
    enable(colorPattern: ColorPatternListDto) {
        this.message.confirm(
            this.l('EnableWarningMessage', colorPattern.name), this.l('Enable'), (result) => {
                if (result) {
    ...
    disable(colorPattern: ColorPatternListDto) {
        this.message.confirm(
            this.l('DisableWarningMessage', colorPattern.name), this.l('Disable'), (result) => {
                if (result) {
    ...
    setAsDefault(colorPattern: ColorPatternListDto) {
        this.message.confirm(
            this.l('DefaultWarningMessage', colorPattern.name), this.l('SetAsDefault'), (result) => {
                if (result) {
    ...
    showInlineActions(colorPattern: ColorPatternListDto, event: Event) {
        if (!this.inlineActionMenu) return;

        this.inlineActionMenu.model = [];
        if (this.canView) this.inlineActionMenu.model.push({ label: this.l('View'), icon: 'pi pi-fw pi-eye', command: () => { this.viewDetail(colorPattern); } });
        if (this.canEdit) this.inlineActionMenu.model.push({ label: this.l('Edit'), icon: 'pi pi-fw pi-pencil', command: () => { this.editColorPattern(colorPattern); } });
        if (this.canDelete) this.inlineActionMenu.model.push({ label: this.l('Delete'), icon: 'pi pi-trash', command: () => { this.delete(colorPattern); } });
