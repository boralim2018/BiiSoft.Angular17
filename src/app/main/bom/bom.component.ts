...
export class BOMComponent extends Mixin(PrimeNgListComponentBase<BOMListDto>, ExcelFileComponentBase, NavBarComponentBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('bomTable') table: Table;
    canCreate: boolean = this.isGranted(AppPermissions.pages.setup.items.boms.create);
    ...
    canView: boolean = this.isGranted(AppPermissions.pages.setup.items.boms.view);
    ...
    canImportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.boms.importExcel);
    canExportExcel: boolean = this.isGranted(AppPermissions.pages.setup.items.boms.exportExcel);
    ...
    ngOnInit() {
        this.tableCacheKey = "BOMTableCacheKey";
        super.ngOnInit();
        this.initNavBar();
    ...
    initNavBar() {
        this.title = this.l("BOMs");
        this.setTitle();
    }

    private initActionMenuItems() {
        this.actionMenuItems = [];
        if (this.canCreate) this.actionMenuItems.push({ label: this.l('Create'), icon: 'pi pi-plus-circle', command: () => { this.createBOM(); } });
    ...
    protected initFilterInput() {
        ...
        this.filterInput.typeFilter = new BOMTypeFilterInputDto({ exclude: false, ids: [] });
        this.filterInput.itemFilter = new GuidFilterInputDto({ exclude: false, ids: [] });
    ...
    protected getList(input: any, callBack: Function) {
        ...
        this._bomService
            .getList(input.typeFilter.exclude, input.typeFilter.ids, input.itemFilter.exclude, input.itemFilter.ids, input.isActive, input.creatorFilter.exclude, input.creatorFilter.ids, input.modifierFilter.exclude, input.modifierFilter.ids, input.keyword, input.sortField, input.sortMode, input.usePagination, input.skipCount, input.maxResultCount)
    ...
    delete(bom: BOMListDto): void {
        ...
        .onClose.subscribe(result => {
            if (result) {
                ...
                this._bomService.delete(bom.id)
    ...
    createBOM(): void {
        this._router.navigate(['/app/main/bom/create']);
    }
    ...
    importExcel() {
        ...
        instance.upload.subscribe(result => {
            if (result) {
                ...
                this._bomService.importExcel(fileInput)
        ...
        instance.download.subscribe(result => {
            instance.loading = true;
            this._bomService.exportExcelTemplate()
                .pipe(finalize(() => instance.loading = false))
                .subscribe(result => {
    ...
    exportExcel() {
        ...
        let input = new ExportExcelBOMInputDto();
        ...
        this._bomService
            ...
            .subscribe((result: ExportFileOutput) => {
                this.downloadExcel(AppConsts.remoteServiceBaseUrl + result.fileUrl, `BOM_${moment().format("YYYY-MM-DD-HH-mm-ss")}.xlsx`);
            });
    }

    editBOM(bom: BOMListDto): void {
        this._router.navigate(['/app/main/bom/edit', bom.id]);
    }

    showCreateOrEditUserDialog(id?: string): void {
        let createOrEditUserDialog: DynamicDialogRef;
        if (!id) {
            createOrEditUserDialog = this._dialogService.open(CreateBOMComponent, {
                data: {},
                header: this.l('Create') + ' ' + this.l('BOM'),
                styleClass: this.responsiveDialogClass
            });
        } else {
            createOrEditUserDialog = this._dialogService.open(EditBOMComponent, {
                data: { id: id },
    ...
    enable(bom: BOMListDto) {
        this.message.confirm(
            this.l('EnableWarningMessage', bom.name), this.l('Enable'), (result) => {
                if (result) {
    ...
    disable(bom: BOMListDto) {
        this.message.confirm(
            this.l('DisableWarningMessage', bom.name), this.l('Disable'), (result) => {
                if (result) {
    ...
    setAsDefault(bom: BOMListDto) {
        this.message.confirm(
            this.l('DefaultWarningMessage', bom.name), this.l('SetAsDefault'), (result) => {
                if (result) {
    ...
    unsetAsDefault(bom: BOMListDto) {
        this.message.confirm(
            this.l('DefaultWarningMessage', bom.name), this.l('UnsetAsDefault'), (result) => {
                if (result) {
    ...
    viewDetail(bom: BOMListDto) {
        this._router.navigate(['/app/main/bom/view-detail', bom.id]);
    }

    showInlineActions(bom: BOMListDto, event: Event) {
        if (!this.inlineActionMenu) return;

        this.inlineActionMenu.model = [];
        if (this.canView) this.inlineActionMenu.model.push({ label: this.l('View'), icon: 'pi pi-fw pi-eye', command: () => { this.viewDetail(bom); } });
        if (this.canEdit) this.inlineActionMenu.model.push({ label: this.l('Edit'), icon: 'pi pi-fw pi-pencil', command: () => { this.editBOM(bom); } });
        if (this.canDelete) this.inlineActionMenu.model.push({ label: this.l('Delete'), icon: 'pi pi-trash', command: () => { this.delete(bom); } });
        if (this.canEnable && !bom.isActive) this.inlineActionMenu.model.push({ label: this.l('Enable'), icon: 'pi pi-check', command: () => { this.enable(bom); } });
        if (this.canDisable && bom.isActive) this.inlineActionMenu.model.push({ label: this.l('Disable'), icon: 'pi pi-ban', command: () => { this.disable(bom); } });
        if (this.canSetAsDefault && !bom.isDefault) this.inlineActionMenu.model.push({ label: this.l('SetAsDefault'), icon: 'fa-solid fa-check-double', command: () => { this.setAsDefault(bom); } });
        if (this.canSetAsDefault && bom.isDefault) this.inlineActionMenu.model.push({ label: this.l('UnsetAsDefault'), icon: 'fa-solid fa-check-double', command: () => { this.unsetAsDefault(bom); } });
    ...
    onItemsChange(event) {
        this.filterInput.itemFilter.ids = !event ? undefined : Array.isArray(event) ? event.map(f => f.id) : [event.id];
    }
