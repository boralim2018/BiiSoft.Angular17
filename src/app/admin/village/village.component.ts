...
@Component({
    ...
    standalone: true,
    imports: [MenuModule, SidebarModule, NgClass, ButtonDirective, Ripple, FormsModule, InputTextModule, DropdownModule, FindCountryComponent, FindCityProvinceComponent, FindKhanDistrictComponent, FindSangkatCommuneComponent, FindUserComponent, SearchFooterComponent, OverlayPanelModule, TableSettingComponent, NavBarComponent, SearchActionComponent, TableModule, PrimeTemplate, NgStyle, NgFor, NgIf, TagModule, RecordNotFoundComponent, DatePipe]
})
export class VillageComponent extends Mixin(PrimeNgListComponentBase<VillageListDto>, ExcelFileComponentBase, NavBarComponentBase) implements OnInit {

    protected get sortField(): string { return 'Code'; }

    @ViewChild('villageTable') table: Table;
    canCreate: boolean = this.isGranted(AppPermissions.pages.setup.locations.villages.create);
    ...
    canView: boolean = this.isGranted(AppPermissions.pages.setup.locations.villages.view);
    ...
    canImportExcel: boolean = this.isGranted(AppPermissions.pages.setup.locations.villages.importExcel);
    canExportExcel: boolean = this.isGranted(AppPermissions.pages.setup.locations.villages.exportExcel);
    ...
    ngOnInit() {
        this.tableCacheKey = "VillageTableCacheKey";
        super.ngOnInit();
        this.initNavBar();
    ...
    initNavBar() {
        this.title = this.l("Villages");
        this.setTitle();
    }

    private initActionMenuItems() {
        this.actionMenuItems = [];
        if (this.canCreate) this.actionMenuItems.push({ label: this.l('Create'), icon: 'pi pi-plus-circle', command: () => { this.createVillage(); } });
    ...
    delete(village: VillageListDto): void {

        this._dialogService.open(ConfirmDeleteComponent, {
            data: {
                deleteObj: village.code + " - " + village.name,
    ...
    exportExcel() {
        ...
        let input = new ExportExcelVillageInputDto();
    ...
    editVillage(village: VillageListDto): void {
        this.showCreateOrEditUserDialog(village.id);
    }

    showCreateOrEditUserDialog(id?: string): void {
        let createOrEditUserDialog: DynamicDialogRef;
        if (!id) {
            createOrEditUserDialog = this._dialogService.open(CreateVillageComponent, {
                data: {},
                header: this.l('Create') + ' ' + this.l('Village'),
                styleClass: this.responsiveDialogClass
            });
        } else {
            createOrEditUserDialog = this._dialogService.open(EditVillageComponent, {
                data: { id: id },
    ...
    enable(village: VillageListDto) {
        this.message.confirm(
            this.l('EnableWarningMessage', village.name), this.l('Enable'), (result) => {
                if (result) {
    ...
    disable(village: VillageListDto) {
        this.message.confirm(
            this.l('DisableWarningMessage', village.name), this.l('Disable'), (result) => {
                if (result) {
    ...
    viewDetail(village: VillageListDto) {
        this._router.navigate(['/app/admin/villages/view-detail', village.id]);
    }

    showInlineActions(village: VillageListDto, event: Event) {
        if (!this.inlineActionMenu) return;

        this.inlineActionMenu.model = [];
        if (this.canView) this.inlineActionMenu.model.push({ label: this.l('View'), icon: 'pi pi-fw pi-eye', command: () => { this.viewDetail(village); } });
        if (this.canEdit) this.inlineActionMenu.model.push({ label: this.l('Edit'), icon: 'pi pi-fw pi-pencil', command: () => { this.editVillage(village); } });
        if (this.canDelete) this.inlineActionMenu.model.push({ label: this.l('Delete'), icon: 'pi pi-trash', command: () => { this.delete(village); } });
        if (this.canEnable && !village.isActive) this.inlineActionMenu.model.push({ label: this.l('Enable'), icon: 'pi pi-check', command: () => { this.enable(village); } });
        if (this.canDisable && village.isActive) this.inlineActionMenu.model.push({ label: this.l('Disable'), icon: 'pi pi-ban', command: () => { this.disable(village); } });
