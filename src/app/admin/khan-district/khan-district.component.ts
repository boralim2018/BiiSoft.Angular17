...
export class KhanDistrictComponent extends Mixin(PrimeNgListComponentBase<KhanDistrictListDto>, ExcelFileComponentBase, NavBarComponentBase) implements OnInit {
    ...
    protected getList(input: any, callBack: Function) {
        ...
        this._khanDistrictService
            .getList(input.countries.exclude, input.countries.ids, input.cityProvinces.exclude, input.cityProvinces.ids, input.isActive, input.creatorFilter.exclude, input.creatorFilter.ids, input.modifierFilter.exclude, input.modifierFilter.ids, input.keyword, input.sortField, input.sortMode, input.usePagination, input.skipCount, input.maxResultCount)
