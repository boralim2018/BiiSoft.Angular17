...
import { PageItemSizeInputDto, FindItemSizeDto, ItemSizeServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    selector: 'find-item-size-dialog',
    templateUrl: './find-item-size-dialog.component.html',
...
export class FindItemSizeDialogComponent extends Mixin(FindCardListComponentBase<FindItemSizeDto>, AppDynamicDialogBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('findItemSizeTable') table: Table;
    ...
    constructor(
        ...
        this.tableCacheKey = "findItemSizeTableCache";
        this.containerClass = '.find-item-size-dialog';
    }
    ...
    protected getList(input: any, callBack: Function): void {
        ...
        let findInput = new PageItemSizeInputDto();
        findInput.init(input);

        this.isTableLoading = true;

        this._itemSizeService.find(findInput)
    ...
    select(itemSize?: any) {
        if (itemSize) {
            this._dialogRef.close(itemSize);
            return;
        }

        let selected = this.selectedModel;
        if (!selected) {
            this.message.warn(this.l("PleaseSelect_", this.l("ItemSize")));
            return;
        }
