...
import { PageItemSeriesInputDto, FindItemSeriesDto, ItemSeriesServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    selector: 'find-item-series-dialog',
    templateUrl: './find-item-series-dialog.component.html',
...
export class FindItemSeriesDialogComponent extends Mixin(FindCardListComponentBase<FindItemSeriesDto>, AppDynamicDialogBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('findItemSeriesTable') table: Table;
    ...
    constructor(
        ...
        this.tableCacheKey = "findItemSeriesTableCache";
        this.containerClass = '.find-item-series-dialog';
    }
    ...
    protected getList(input: any, callBack: Function): void {
        ...
        let findInput = new PageItemSeriesInputDto();
        findInput.init(input);

        this.isTableLoading = true;

        this._itemSeriesService.find(findInput)
    ...
    select(itemSeries?: any) {
        if (itemSeries) {
            this._dialogRef.close(itemSeries);
            return;
        }

        let selected = this.selectedModel;
        if (!selected) {
            this.message.warn(this.l("PleaseSelect_", this.l("ItemSeries")));
            return;
        }
