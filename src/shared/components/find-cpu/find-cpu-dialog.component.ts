...
import { PageCPUInputDto, FindCPUDto, CPUServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    selector: 'find-cpu-dialog',
    templateUrl: './find-cpu-dialog.component.html',
...
export class FindCPUDialogComponent extends Mixin(FindCardListComponentBase<FindCPUDto>, AppDynamicDialogBase) implements OnInit {
    ...
    constructor(
        ...
        this.tableCacheKey = "findCPUTableCache";
        this.containerClass = '.find-cpu-dialog';
    }
    ...
    protected getList(input: any, callBack: Function): void {
        ...
        let findInput = new PageCPUInputDto();
        findInput.init(input);

        this.isTableLoading = true;

        this._cpuService.find(findInput)
    ...
    select(cpu?: any) {
        ...
        if (!selected) {
            this.message.warn(this.l("PleaseSelect_", this.l("CPU")));
            return;
        }
