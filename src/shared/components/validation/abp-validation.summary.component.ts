...
export class AbpValidationSummaryComponent extends AppComponentBase implements OnInit {
        ...
        if (this.controlEl) {
            this.control.valueChanges.subscribe(() => {
                if (
                    ...
                    this._renderer.removeClass(this.controlEl, 'ng-invalid');
                }
            });
        }
    }
    ...
    getValidationErrorMessage(error: AbpValidationError): string {
        if (this.controlEl) {
            ...
            this._renderer.addClass(this.controlEl, 'ng-invalid');
        }

        if (error.name === 'pattern' && this.controlEl['type'] === 'email') return this.l('InvalidEmailAddress');
        if (error.name === 'pattern' && this.controlEl['type'] === 'url') return this.l('InvalidURL');
