...
@Component({
    selector: 'find-warehouse-dialog',
    templateUrl: './find-warehouse-dialog.component.html',
...
export class FindWarehouseDialogComponent extends Mixin(FindCardListComponentBase<FindWarehouseDto>, AppDynamicDialogBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('findWarehouseTable') table: Table;
    ...
    constructor(
        ...
        this.tableCacheKey = "findWarehouseTableCache";
        this.containerClass = '.find-warehouse-dialog';
    }
    ...
    protected getList(input: any, callBack: Function): void {
        ...
        let findInput = new FindWarehouseInputDto();
        findInput.init(input);

        this.isTableLoading = true;

        this._warehouseService.find(findInput)
    ...
    select(warehouse?: any) {
        if (warehouse) {
            this._dialogRef.close(warehouse);
            return;
        }

        let selected = this.selectedModel;
        if (!selected) {
            this.message.warn(this.l("PleaseSelect_", this.l("Warehouse")));
            return;
        }
