...
import { PageBranchInputDto, FindBranchDto, BranchServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    selector: 'find-branch-dialog',
    templateUrl: './find-branch-dialog.component.html',
...
export class FindBranchDialogComponent extends Mixin(FindCardListComponentBase<FindBranchDto>, AppDynamicDialogBase) implements OnInit {
    ...
    constructor(
        ...
        this.tableCacheKey = "findBranchTableCache";
        this.containerClass = '.find-branch-dialog';
    }
    ...
    protected getList(input: any, callBack: Function): void {
        ...
        let findInput = new PageBranchInputDto();
        findInput.init(input);

        this.isTableLoading = true;

        this._branchService.find(findInput)
    ...
    select(branch?: any) {
        ...
        if (!selected) {
            this.message.warn(this.l("PleaseSelect_", this.l("Branch")));
            return;
        }
