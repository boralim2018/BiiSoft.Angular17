...
export class FindItemGroupDialogComponent extends Mixin(FindCardListComponentBase<FindItemGroupDto>, AppDynamicDialogBase) implements OnInit {

    protected get sortField(): string { return 'Name'; }

    @ViewChild('findItemGroupTable') table: Table;
    ...
    constructor(
        ...
        this.tableCacheKey = "findItemGroupTableCache";
        this.containerClass = '.find-item-group-dialog';
    }
    ...
    protected getList(input: any, callBack: Function): void {
        ...
        let findInput = new PageItemGroupInputDto();
        findInput.init(input);

        this.isTableLoading = true;

        this._itemGroupService.find(findInput)
    ...
    select(itemGroup?: any) {
        if (itemGroup) {
            this._dialogRef.close(itemGroup);
            return;
        }

        let selected = this.selectedModel;
        if (!selected) {
            this.message.warn(this.l("PleaseSelect_", this.l("ItemGroup")));
            return;
        }
